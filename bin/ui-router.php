...
if ($path === '/api/library') {
    ...
    if (is_file($database)) {
        ...
        $query = $pdo->query('SELECT game_id, title, slug FROM games ORDER BY title COLLATE NOCASE');
...
if ($path === '/api/artwork') {
    ...
    $cache = $cacheDir . '/' . $id . '.txt';
    $image = is_file($cache) ? trim((string) file_get_contents($cache)) : '';
    if ($image === '') {
...
function runCommand(string $root): void
{
    ...
    try {
        ...
        ftruncate($lock, 0);
        fwrite($lock, json_encode(['command' => $name, 'startedAt' => date(DATE_ATOM)]));
        fflush($lock);
